...
while True:
    os.system('cls')
    with open('Python-One-Word-Story\\Windows\\Story.txt') as file:
        contents = file.read()
    ...
    with open('Python-One-Word-Story\\Windows\\Names.txt') as file3:
        contents = file3.read()
        search_word = N
        if search_word in contents:
    ...
    if N == 'SeeStory':
        os.system('cls')
        file1 = open('Python-One-Word-Story\\Windows\\Story.txt','r')
        file1.seek(0)
    ...
    if search_word not in contents:
        ...
        L = input()
        file1 = open('Python-One-Word-Story\\Windows\\Story.txt','a')

    if len(L.split()) == 1:
        file2 = open('Python-One-Word-Story\\Windows\\Names.txt','a')
        file2.writelines(N)
        file2.writelines('\n')
        file2.close()
